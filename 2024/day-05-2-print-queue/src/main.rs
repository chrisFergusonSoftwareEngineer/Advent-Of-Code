            ...
            if let Ok(line_string) = line {
                if reading_rules {
                    ...
                    while index < seconds.len() {
                        ...
                        if second_place != None && first_place != None {
                            if second_place.unwrap() < first_place.unwrap() {
                                println!("Pages {:?} failed on rule {}: {} | {}", pages, index, firsts[index], seconds[index]);
                                valid = false;

                                pages[first_place.unwrap()] = seconds[index];
                                pages[second_place.unwrap()] = firsts[index];
                    ...
                    if !valid {
                        ...
                        total += pages[middle];
                        println!("Final: {:?} => {}", pages, pages[middle]);
                    }
                }
            }
        }
